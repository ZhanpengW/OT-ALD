...
def main():
    ...
    scatter(latents_2_new, latents_2_new_image_path)
    ...
    DIS=[]
    M = ot.dist(latents_2, latents_2_new, metric='euclidean')
    # 均匀权重 (假设每个点的概率相等)
    a = np.ones(len(latents_2)) / len(latents_2)  # 源分布权重
    b = np.ones(len(latents_2_new)) / len(latents_2_new)  # 目标分布权重
    ...
    # 加载已有 Excel 文件
    file_path = os.path.join(image_subfolder, 'DIS_LATENT_OT.xlsx')
