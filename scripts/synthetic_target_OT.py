...
def main():
    ...
    latents = []
    sources = []
    data = load_2d_data(n_samples=30000, batch_size=args.batch_size, shape=shape_s, training=False)
    ...
    targets_2 = diffusion.ddim_sample_loop(
        target_model, (30000, 2),
        noise=latents_2_new,
        clip_denoised=False,
        device=dist_util.dev(),
        truncated_step=truncated_step,
    )
    logger.log(f"translated to target {targets_2.shape}")
    latents_2_new=latents_2_new.cpu().numpy()
    targets_2=targets_2.cpu().numpy()
    ...
    sources_image_path = os.path.join(image_subfolder, 'scatter_sources.pdf')
    scatter(sources, sources_image_path)
    latents_image_path = os.path.join(image_subfolder, 'scatter_latents.pdf')
    scatter(latents, latents_image_path)
    latents_2_image_path = os.path.join(image_subfolder, 'scatter_latents_2.pdf')
    scatter(latents_2_new, latents_2_image_path)
    targets_image_path = os.path.join(image_subfolder, 'scatter_targets.pdf')
    scatter(targets, targets_image_path)
    targets_image_path = os.path.join(image_subfolder, 'scatter_targets_2.pdf')
    scatter(targets_2, targets_image_path)
    aaa
    DIS=[]
    M = ot.dist(targets, targets_2, metric='euclidean')
    # 均匀权重 (假设每个点的概率相等)
    a = np.ones(len(targets)) / len(targets)  # 源分布权重
    b = np.ones(len(targets_2)) / len(targets_2)  # 目标分布权重
    # 计算 2D 瓦瑟斯坦距离
    ...
    # 加载已有 Excel 文件
    file_path = os.path.join(image_subfolder, 'DIS_TARGET_OT.xlsx')
