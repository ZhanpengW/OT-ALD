...
def main():
    ...
    args.truncated_step = args.truncated_step*4
    ...
    for i, (batch, extra) in enumerate(data_source):
        ...
        noise_source = diffusion_source.ddim_reverse_sample_loop(
            model_source,
            batch,
            clip_denoised=False,
        ...
        sample_target = diffusion_target.ddim_sample_loop(
            model_target,
        ...
        sample_source = diffusion_source.ddim_sample_loop(
            model_source,
            (args.batch_size, 3, args.image_size, args.image_size),
            noise=noise_target,
            clip_denoised=args.clip_denoised,
        ...
        sample_source = ((sample_source + 1) * 127.5).clamp(0, 255)#.to(th.uint8)
    ...
    DIS=[]
    distance = np.linalg.norm(source - sample_source, axis=1).mean()
    ...
    for i in range(args.batch_size):
        noise_source_i = source[i].flatten()
        noise_target_i = sample_source[i].flatten()
    ...
    # 加载已有 Excel 文件
    wb = load_workbook("experiments/DIS_CYCLE_WILD2DOG.xlsx")
    ...
    # 保存修改
    wb.save("experiments/DIS_CYCLE_WILD2DOG.xlsx")
